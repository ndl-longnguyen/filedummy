...
export function DownloadButton({ fileKey, label, locale = "en" }: DownloadButtonProps) {
    ...
    <div className="w-full">
      ...
      {countdown === null && !isCompleted && (
        ...
        >
          ...
          <span>{isVi ? `Tải Tệp Mẫu ${label}` : `Download ${label} Sample File`}</span>
        </button>
      )}
        ...
        <div className="p-6 rounded-2xl bg-slate-900/90 border border-blue-500/40 shadow-2xl space-y-4">
          <div className="text-center space-y-2">
            ...
            <h4 className="text-lg font-bold text-white">
              {isVi ? "Đang Chuẩn Bị Tệp Tải Xuống..." : "Preparing Your Download..."}
            </h4>
            <p className="text-sm text-slate-400">
              {isVi ? (
                <>
                  Tệp mẫu sẽ tự động bắt đầu tải xuống trong{" "}
                  <span className="text-blue-400 font-semibold">{countdown} giây</span>.
                </>
              ) : (
                <>
          ...
          <div className="my-3">
            <AdUnit slot="9876543210" format="rectangle" label={isVi ? "Đối tác tài trợ" : "Sponsored Partner"} />
          </div>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
            ...
            >
              {isVi ? "Bỏ qua đếm ngược & tải ngay" : "Skip countdown & start directly"}{" "}
      ...
      {isCompleted && (
        <div className="p-6 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 text-center space-y-3">
          ...
          <h4 className="text-base font-bold text-white">
            {isVi ? "Tải Xuống Đã Bắt Đầu!" : "Download Started!"}
          </h4>
          <p className="text-xs text-slate-400">
            {isVi ? (
              <>
                Trình duyệt của bạn sẽ hiển thị tệp tải. Nếu không thấy phản hồi,{" "}
                <a href={downloadUrl} className="text-emerald-400 font-semibold underline hover:text-emerald-300">
                  bấm vào đây để tải thủ công
                </a>.
              </>
            ) : (
              <>
          ...
          >
            ...
            {isVi ? "Tải Lại Lần Nữa" : "Download Again"}
          </button>
        </div>
      )}
    </div>
